...
- deployment_version_source (optional): how the app derives
  `DEPLOYMENT_VERSION` (git SHA, CI build id). Default: `git rev-parse --short HEAD`.
...
- You MUST show the `next.config` diff and get explicit approval before
  writing it — this file frequently contains user customizations.
...
Merge these keys into the existing config — do not replace the file:
...
cacheComponents: true,   // Next 16 only — skip if the user does not want 'use cache'
...
REDIS_URL=redis://127.0.0.1:6379 DEPLOYMENT_VERSION=setup-verify npm run start &
```

Then request any ISR/static page twice and check the `x-nextjs-cache`
header: first `MISS`, second `HIT`. Confirm keys exist:
`redis-cli KEYS 'next-incremental:*'`. Kill the server afterwards.
